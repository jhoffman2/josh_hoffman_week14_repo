...
public class LineDrawer : MonoBehaviour
{
    ...
    void Update()
    {
        //1 on the keyboard selects normal line
        if (Input.GetKeyDown(KeyCode.Alpha1))
        {
            currentLine = linePrefab;
        }

        //2 on the keyboard selects bouncy line
        if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            currentLine = bouncyLinePrefab;
        }
        ...
        {
            _currentLine = Instantiate (currentLine).GetComponent <LineRenderer> ();
